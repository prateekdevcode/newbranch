	...
	public function display_product_options($post_id = false)
	{
		...
		if($pro_type === 'simple' || $pro_type === 'variable')
		{
			...
			foreach ( $array_options as $options_key => $options ) {
				...
				elseif( isset($options['type']) && $options['type'] === 'custom_radio' )
				{
					
					wc_get_template( 'custom_radio.php', array( 'options' => $options ), 'custom-options', plugin_dir_path( dirname( __FILE__ ) )  . '/templates/' );
				
				}
				elseif( isset($options['type']) && $options['type'] === 'custom_multiorder' )
				{
					
					wc_get_template( 'custom_multiorder.php', array( 'options' => $options ), 'custom-options', plugin_dir_path( dirname( __FILE__ ) )  . '/templates/' );
				...
				elseif( isset($options['type']) && $options['type'] === 'custom_file' )
				{
					
					wc_get_template( 'custom_file.php', array( 'options' => $options ), 'custom-options', plugin_dir_path( dirname( __FILE__ ) )  . '/templates/' );
