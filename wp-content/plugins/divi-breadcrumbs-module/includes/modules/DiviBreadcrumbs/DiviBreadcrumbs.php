...
class dcsbcm_Divi_Breadcrumbs_Module extends ET_Builder_Module {
	...
	protected $module_credits = array(
		...
		'author'     => 'Divi Codex',
		'author_uri' => 'https://divicake.com/author/divicodex/',
	);
	...
	public function init() {
		...
		$this->custom_css_fields = array(
			...
			'breadcrumbcss_separators' => array(
	...
	public function get_fields() {
		...
		$module_fields['category_breadcrumb_text'] = [
			'label'           => esc_html__( 'Category Breadcrumb Text', 'et_builder' ),
			'type'            => 'text',
			'option_category' => 'basic_option',
			'depends_show_if' => 'off',
			'description'     => esc_html__( 'If you would like to override the "Category" breadcrumb text, input your own text here. The word "Category" will be shown on category pages if this field is left blank.', 'et_builder' ),
			'toggle_slug'     => 'breadcrumbnav_settings',
			'computed_affects' => array(
				'computed_field_html_payload',
			),
		...
		$module_fields['computed_field_html_payload'] = [
			'type'					=> 'computed',
			'computed_callback' 	=> array( 'dcsbcm_Divi_Breadcrumbs_Module', 'dcsbcm_build_output' ),
	...
	static function dcsbcm_build_output( $args = array(), $conditional_tags = array(), $current_page = array() ) {
		...
		$category_breadcrumb_text					= $args['category_breadcrumb_text'];
		...
		$cat_text=' "%s" ';
		if($category_breadcrumb_text==''){
			$breadcrumbFullText['category'] = "Category: $cat_text"; // text for a category page
		}
		else{
			$breadcrumbFullText['category'] = "$category_breadcrumb_text:$cat_text"; // text for a category page
		}
		...
		$linkBefore = '<li class="dcsbcm_divi_breadcrumb" property="itemListElement" typeof="ListItem">';
		$linkAfter = '<meta property="position" content="1"></li>';
		$linkAttr = ' property="item" typeof="WebPage"';
		...
		if ( is_home() || is_front_page() ) {
			...
			$dcsbcm_Output_BreadcrumbsHTML = '<ol class="dcsbcm_divi_breadcrumbs" vocab="http://schema.org/" typeof="BreadcrumbList">';
			...
			if ( is_category() ) {
				...
				$position=$position+1;
				$dcsbcm_Output_BreadcrumbsHTML .= sprintf($link, get_year_link(get_the_time('Y')), get_the_time('Y'),$position) . $separator;
				$dcsbcm_Output_BreadcrumbsHTML .= sprintf($link, get_month_link(get_the_time('Y'),get_the_time('m')), get_the_time('F'),$position) . $separator;
			...
			$dcsbcm_Output_BreadcrumbsHTML .= '</ol>';
	...
	public function get_advanced_fields_config() {
		...
		$advanced_fields['fonts'] = [
			...
			'fontsbreadcrumblinks'   => [
				'label'    => esc_html__( 'Breadcrumb Links', 'et_builder' ),
				'css'      => array(
					'main' => "{$this->main_css_element} li.dcsbcm_divi_breadcrumb a",
				),
			],
		];
	...
	public function render( $attrs, $content = null, $render_slug ) {
		...
		$category_breadcrumb_text					= $this->props['category_breadcrumb_text'];
		...
		$linkAfter = '</li>';
		$linkAttr = ' property="item" typeof="WebPage"';
		$link = $linkBefore . '<a' . $linkAttr . ' href="%1$s"><span property="name">%2$s</span></a><meta property="position" content="%3$s">' . $linkAfter;
		$position=1;
		if ( is_home() || is_front_page() ) {
			...
			$dcsbcm_Output_BreadcrumbsHTML = '<ol class="dcsbcm_divi_breadcrumbs" vocab="http://schema.org/" typeof="BreadcrumbList">';
			// $position=$position+1;
			if ( $hide_homebreadcrumb == 0 ) { $dcsbcm_Output_BreadcrumbsHTML .= sprintf( $link, $homeLink, $breadcrumbFullText['home'],$position) . $separator; }
			
			if ( is_category() ) {
				$thisCat = get_category(get_query_var('cat'), false);
				if ($thisCat->parent != 0) {
					$cats = get_category_parents($thisCat->parent, TRUE, $separator);
					$cats = preg_replace('/(<a\b[^><]*)>/i', '$1 toAddPropertyName>', $cats);
					$cats = str_replace('<a', $linkBefore . '<a' . $linkAttr, $cats);
					$cats = str_replace('toAddPropertyName>','><span property="name">', $cats);
					$position=$position+1;
					$cats = str_replace('</a>', '</span></a><meta property="position" content="'.$position.'">' . $linkAfter, $cats);					
					$dcsbcm_Output_BreadcrumbsHTML .= $cats;
				}
				$dcsbcm_Output_BreadcrumbsHTML .= $breadcrumbActiveBefore . sprintf($breadcrumbFullText['category'], single_cat_title('', false)) . $breadcrumbActiveAfter;
				...
				$position=$position+1;
				$dcsbcm_Output_BreadcrumbsHTML .= sprintf($link, get_year_link(get_the_time('Y')), get_the_time('Y'),$position) . $separator;
				$dcsbcm_Output_BreadcrumbsHTML .= sprintf($link, get_month_link(get_the_time('Y'),get_the_time('m'),$position), get_the_time('F')) . $separator;
				$dcsbcm_Output_BreadcrumbsHTML .= $breadcrumbActiveBefore . get_the_time('d') . $breadcrumbActiveAfter;
				...
				if ( get_post_type( $current_page_ID ) != 'post' ) {
					...
					$slug = $post_type->rewrite;
					$position=$position+1;
					$dcsbcm_Output_BreadcrumbsHTML .= sprintf($link, $homeLink . '/' . $slug['slug'] . '/', $post_type->labels->singular_name,$position);
					if ($hide_currentbreadcrumb == 0) $dcsbcm_Output_BreadcrumbsHTML .= $separator . $breadcrumbActiveBefore . get_the_title( $current_page_ID ) . $breadcrumbActiveAfter;
				} else {
					$cat = get_the_category(); $cat = $cat[0];
					$position=$position+1;
				...
				$dcsbcm_Output_BreadcrumbsHTML .= $cats;
				$position=$position+1;
				$dcsbcm_Output_BreadcrumbsHTML .= sprintf($link, get_permalink($parent), $parent->post_title,$position);
				if ($hide_currentbreadcrumb == 0) $dcsbcm_Output_BreadcrumbsHTML .= $separator . $breadcrumbActiveBefore . get_the_title( $current_page_ID ) . $breadcrumbActiveAfter;
				...
				while ($parent_id) {
					$position=$position+1;
					$page = get_post($parent_id);
					$breadcrumbs[] = sprintf($link, get_permalink($page->ID), get_the_title($page->ID),$position);
					$parent_id  = $page->post_parent;
				}
				$breadcrumbs = array_reverse($breadcrumbs);
				for ($i = 0; $i < count($breadcrumbs); $i++) {
