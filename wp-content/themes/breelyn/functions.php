...
function custom_cart_items_prices( $cart ) {
    ...
    foreach ( $cart->get_cart() as $cart_item ) {
        ...
        // SET THE NEW NAME
        $new_name = str_replace("Polo Short Sleeve - ","",$original_name);
...
function update_custom_price( $cart_object ) {
    ...
    foreach ( $cart_object->cart_contents as $cart_item_key => $value ) { 
      ...
      $terms = wp_get_post_terms( $value['data']->id, 'product_cat' );
      ...
      if ( in_array( 'custom-products', $categories ) ) {
        ...
        // $terms = get_terms ( $value['data']->id, 'product_cat' );
        // foreach ( $terms as $term ) {
        //      echo $term->id.'<br>';
        // }
        // foreach ($value['data']->category_ids as $key => $cID) {
        //   echo get_the_category_by_ID($cID).'<br>';
        // }
        //$term_obj_list = get_the_terms( $post->ID, 'taxonomy' );
       // die();
        $pocket_price = 2.2;
        //echo '<br>'.($value['key']);
        if( $value['woo_discount']['additional_details'][0]['discount_type'] == 'fixed_price'){

          $discount_value = $value['woo_discount']['additional_details'][0]['discount_value'] ;
          $discount_quantity = $value['quantity'] ;
          
          if((isset($value['tinvwl_formdata']['custom-options']['pocket_option']) && ($value['tinvwl_formdata']['custom-options']['pocket_option'] != '')) &&  $value['tinvwl_formdata']['custom-options']['pocket_option'] != 'no-pocket'){
           
            $discount_value = $discount_value + 2.2;
          }

          //$discountedPrice = $discount_quantity * $discount_value;
          // echo "discount_value ".$discount_value;
          // echo "<br>discount_quantity ".$discount_quantity;
       ...
       $value['data']->set_price($discount_value);

      

     //WC()->cart->remove_cart_item('e36ea5a81d24b72960c660e6cc22e356');
      
        // if(isset($value['tinvwl_formdata']['custom-options']['pocket_option']) && $value['tinvwl_formdata']['custom-options']['pocket_option'] != 'no-pocket'){
        //  // echo "p ".$discountedPrice;
        // //  $value['data']->set_price(number_format($discountedPrice,2) + ($pocket_price * $value['quantity']));
            
        // }else{
        //   //echo "p ".$discountedPrice;
        //  //$value['data']->set_price(number_format($discountedPrice,2));
        // } 
      }
    }
}
...
function shadow_images_html( $post) {
	...
	<p>
		<label for="shadow_images_shadow_1"><?php _e( 'Shadow 1', 'shadow_images' ); ?></label><br>
		<input type="file" name="shadow_images_shadow_1" id="shadow_images_shadow_1" value="">
	</p>	
	<?php
	$shadow_images_shadow_1 = shadow_images_get_meta( 'shadow_images_shadow_1' );	
	if ( $shadow_images_shadow_1 != '' ) { ?>
	     <div><p><img src="<?php echo $shadow_images_shadow_1; ?>" width="85px" ></p></div>
	<?php } ?>
	<p>
		<label for="shadow_images_shadow_2"><?php _e( 'Shadow 2', 'shadow_images' ); ?></label><br>
		<input type="file" name="shadow_images_shadow_2" id="shadow_images_shadow_1" value="">
	</p>	
	<?php
	$shadow_images_shadow_2 = shadow_images_get_meta( 'shadow_images_shadow_2' );	
	if ( $shadow_images_shadow_2 != '' ) { ?>
	     <div><p><img src="<?php echo $shadow_images_shadow_2; ?>" width="85px" ></p></div>
	<?php } ?>
	
	<p>
		<label for="shadow_images_shadow_3"><?php _e( 'Shadow 3', 'shadow_images' ); ?></label><br>
		<input type="file" name="shadow_images_shadow_3" id="shadow_images_shadow_3" value="">
	</p>	
	<?php
	$shadow_images_shadow_3 = shadow_images_get_meta( 'shadow_images_shadow_3' );	
	if ( $shadow_images_shadow_3 != '' ) { ?>
	     <div><p><img src="<?php echo $shadow_images_shadow_3; ?>" width="85px" ></p></div>
	<?php } ?>
	
	<p>
		<label for="shadow_images_shadow_4"><?php _e( 'Shadow 4', 'shadow_images' ); ?></label><br>
		<input type="file" name="shadow_images_shadow_4" id="shadow_images_shadow_4" value="">
	</p>	
	<?php
	$shadow_images_shadow_4 = shadow_images_get_meta( 'shadow_images_shadow_4' );	
	if ( $shadow_images_shadow_4 != '' ) { ?>
	     <div><p><img src="<?php echo $shadow_images_shadow_4; ?>" width="85px" ></p></div>
	<?php }
}

add_action( 'save_post', 'shadow_images_save' );
function shadow_images_get_meta( $value ) {
	global $post;
...
function shadow_images_save( $post_id ) {
	...
	if ( !empty($_FILES['shadow_images_shadow_1']['name']) || !empty($_FILES['shadow_images_shadow_2']['name']) || !empty($_FILES['shadow_images_shadow_3']['name']) || !empty($_FILES['shadow_images_shadow_4']['name']) ) {
		$cate = wp_get_post_terms($post_id,'product_cat');
		$slug = get_post_field( 'post_name', $post_id );
		$path = $_SERVER['DOCUMENT_ROOT'].'/wp-content/themes/breelyn/customizer_images/'.$slug.'/';
		if(!is_dir($_SERVER['DOCUMENT_ROOT'].'/wp-content/themes/breelyn/customizer_images/'.$slug)){
			mkdir($_SERVER['DOCUMENT_ROOT'].'/wp-content/themes/breelyn/customizer_images/'.$slug);      
		}
	}
	
	if ( !empty( $_FILES['shadow_images_shadow_1']['name'] ) ) {
		$supported_types = array("jpg", "png", "gif", "bmp", "jpeg", "pdf"); // Supported file types
		
		$extension = pathinfo( $_FILES['shadow_images_shadow_1']['name'], PATHINFO_EXTENSION );
		$name = "1_1.".$extension;
		if ( in_array( $extension, $supported_types ) ) {		
			move_uploaded_file($_FILES["shadow_images_shadow_1"]["tmp_name"], $path.$name);
			$image_url = "https://breelynuniforms.com.au/wp-content/themes/breelyn/customizer_images/".$slug."/".$name;
			add_post_meta( $post_id, 'shadow_images_shadow_1', $image_url );
			update_post_meta( $post_id, 'shadow_images_shadow_1', $image_url );
		}
	}
	
	if ( ! empty( $_FILES['shadow_images_shadow_2']['name'] ) ) {
		$supported_types = array("jpg", "png", "gif", "bmp", "jpeg", "pdf"); // Supported file types	
		$extension = pathinfo( $_FILES['shadow_images_shadow_2']['name'], PATHINFO_EXTENSION );
		$name = "2_2.".$extension;
		if ( in_array( $extension, $supported_types ) ) {		
			move_uploaded_file($_FILES["shadow_images_shadow_2"]["tmp_name"], $path.$name);
			$image_url = "https://breelynuniforms.com.au/wp-content/themes/breelyn/customizer_images/".$slug."/".$name;
			add_post_meta( $post_id, 'shadow_images_shadow_2', $image_url );
			update_post_meta( $post_id, 'shadow_images_shadow_2', $image_url );
		}
	}
	
	if ( ! empty( $_FILES['shadow_images_shadow_3']['name'] ) ) {
		$supported_types = array("jpg", "png", "gif", "bmp", "jpeg", "pdf"); // Supported file types
		$name = "3_3.".$extension;
		$extension = pathinfo( $_FILES['shadow_images_shadow_3']['name'], PATHINFO_EXTENSION );
		if ( in_array( $extension, $supported_types ) ) {
			move_uploaded_file($_FILES["shadow_images_shadow_3"]["tmp_name"], $path.$name);
			$image_url = "https://breelynuniforms.com.au/wp-content/themes/breelyn/customizer_images/".$slug."/".$name;
			add_post_meta( $post_id, 'shadow_images_shadow_3', $image_url );
			update_post_meta( $post_id, 'shadow_images_shadow_3', $image_url );
		}
	}
	
	if ( ! empty( $_FILES['shadow_images_shadow_4']['name'] ) ) {
		$supported_types = array("jpg", "png", "gif", "bmp", "jpeg", "pdf"); // Supported file types
		//$name = $_FILES['shadow_images_shadow_4']['name'];
		$extension = pathinfo( $_FILES['shadow_images_shadow_4']['name'], PATHINFO_EXTENSION );
		$name = "4_4.".$extension;
		if ( in_array( $extension, $supported_types ) ) {		
			move_uploaded_file($_FILES["shadow_images_shadow_4"]["tmp_name"], $path.$name);
			$image_url = "https://breelynuniforms.com.au/wp-content/themes/breelyn/customizer_images/".$slug."/".$name;
			add_post_meta( $post_id, 'shadow_images_shadow_4', $image_url );
			update_post_meta( $post_id, 'shadow_images_shadow_4', $image_url );
		}
	}
	
	//update_post_meta( $post_id, 'shadow_images_shadow_1', esc_attr( $_POST['shadow_images_shadow_1'] ) );
}

/*
	Usage: shadow_images_get_meta( 'shadow_images_shadow_1' )
*/
...
add_action( 'wp_ajax_nopriv_deleteItemAJAX_callback', 'deleteItemAJAX_callback' );
add_action( 'wp_ajax_deleteItemAJAX_callback', 'deleteItemAJAX_callback' );

function deleteItemAJAX_callback()
{
   global $wpdb;
   $id = $_REQUEST['id'];
   $my_thing = array();
  // $table = 'wp_woo_savedproduct_details';
   //print_r($wpdb->delete( $table, array( 'ID' => $id ) ));

   $res = $wpdb->query("DELETE FROM wp_woo_savedproduct_details WHERE ID = ".$id );
  
  $my_thing[0]['query']="DELETE FROM wp_woo_savedproduct_details WHERE ID = ".$id ;
...
$message = 'Product has been added to your cart. <a href="https://breelynuniforms.com.au/cart/" tabindex="1" class="button wc-forward">View cart</a>' ;
return $message;
}
...
//add_action('wp_footer', 'output_to_footer');
...
function divi_engine_sku_cart_page( $item_name, $cart_item, $cart_item_key  ) {
    ...
    // Add the sku
    $item_name .= '<small class="product-sku">' . __( "Product Code: ", "woocommerce") . $sku . '</small>';
