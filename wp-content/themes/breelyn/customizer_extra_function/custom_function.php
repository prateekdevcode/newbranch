...
function getselectproductstyle_callback(){		
	...
	if($rows)
	{
		...
		$post_id = $wpdb->get_results("SELECT ID FROM wp_posts WHERE (post_title LIKE '%".$rows['design_style_name']."%' AND post_status = 'publish' AND post_type = 'product_variation' AND post_parent = '". $pid ."')");

		$datastyle[] = array('product_variation' => $post_id[0]->ID , 'products-id'=>$loop->post->ID,'data-imagename'=>$rows['design_style_name'],'data-price'=>$rows['design_layer_price'],'data-layer'=>$rows['design_layer'],'default'=>$rows['design_layer_name'],'image'=>$rows['design_main_image']);
...
function save_custom_tshirt_images_callback()
{
	...
	$file = $_SERVER['DOCUMENT_ROOT'].'/wp-content/themes/breelyn-theme/custom_image/'.$filename.'.png';
...
function questiondatahtml_update() {
	if( $_FILES ){
		...
		$path = $_SERVER['DOCUMENT_ROOT'].'/wp-content/themes/breelyn/customizer_images/user-uploaded-logo/';
...
/**********************************************/
	...
	function get_varietionidof_product_callback(){
		...
		$current_product_id = $_POST['currentProID'];
	  ...
	  // Get variations
        $args = array(
			...
			$response['pre_design_template'] = get_field("pre_designed_template",$current_product_id);
...
function savesettingsdata_callback(){
	...
	if(is_user_logged_in()){
		...
		// get previous stored items list

		$query = 'SELECT * FROM wp_woo_savedproduct_details  WHERE user_id ='.$current_user.' AND product_id = '.$get_selectd_data['product_id'].' ORDER BY `wp_woo_savedproduct_details`.`ID` DESC';
		$tot_prev = $wpdb->get_results($query);

		
		
		$total_stored = count($tot_prev);

		/*****************************/
		$file_dir = WP_CONTENT_DIR . '/uploads/design_product/';
		if(!file_exists($file_dir))mkdir($file_dir);
		
		$product_design = $get_selectd_data['product_id'].'_'.$current_user.'_savedata_'.($total_stored + 1);
		...
		// for non logged user based on ip


		$query = 'SELECT * FROM wp_woo_savedproduct_details  WHERE `wp_woo_savedproduct_details`.`user_ip` = "'.$client_ip.'" AND product_id = '.$get_selectd_data['product_id'].' ORDER BY `wp_woo_savedproduct_details`.`ID` DESC';
		$tot_prev = $wpdb->get_results($query);
		...
		$product_design = $get_selectd_data['product_id'].'_'.$client_ip.'_savedata_'.($total_stored + 1);
		...
		$d = $wpdb->insert('wp_woo_savedproduct_details',$asd_insert);
...
function savesettingsdata_show_callback(){
	...
	$response['raw'] = $variable_value;
		...
		$query = 'SELECT * FROM wp_woo_savedproduct_details  WHERE user_ip = "'.$client_ip.'" AND product_id = '.$product_id.' ORDER BY `wp_woo_savedproduct_details`.`ID` DESC LIMIT 0,10';
		$result = $wpdb->get_results($query);
		$html = '<ul>';
		foreach ($result as $key => $result_itm) {
			$currentDesign = $result_itm->current_design;
			$itm_img = file_get_contents($currentDesign);
			$html .= '<li> <span><img src="'. $itm_img .'"></span> <span><b>Created: </b>'. $result_itm->created .'</span>
			<a class="btn btn-primary download_now" download="'.$result_itm->created.'.png" href="'.$itm_img.'" data-id="'.$result_itm->ID.'"  ><i class="fa fa-download" style="margin-right:15px"></i>Download</a></li>';
		}
		$html .= '</ul>';
...
add_action( 'wp_ajax_custom_option_file_upload', 'custom_option_file_upload_callback' );
add_action( 'wp_ajax_nopriv_custom_option_file_upload', 'custom_option_file_upload_callback' );
function custom_option_file_upload_callback() {
	if( $_FILES ){
		$valid_formats = array("jpg", "png", "gif", "bmp", "jpeg", "pdf"); // Supported file types
		$max_file_size = 5 * 1048576; // in kb	
		$path = $_SERVER['DOCUMENT_ROOT'].'/wp-content/themes/breelyn/customizer_images/user-uploaded-logo/';
			  ...
			  $arr =array('suc'=>0,'msg'=>$name." is not a valid format");
			   ...
			   exit;
				...
				if(move_uploaded_file($_FILES["file"]["tmp_name"], $path.$new_filename)){
				  $arr =array("status"=>"ok","name"=>$new_filename);
				  echo json_encode($arr);
				} else {
					header('HTTP/1.1 500 Internal Server');
					header('Content-Type: application/json; charset=UTF-8');
...
add_action( 'woocommerce_update_cart_action_cart_updated', 'on_action_cart_updated', 10, 1 );
function on_action_cart_updated( $cart_updated ){
	...
	foreach ( $woocommerce->cart->get_cart() as $cart_item_key => $cart_item ) {
		...
		foreach($data as $key => $value) {
			...
			if($key == $cart_item_key && $value != '') { 
                $nCal =  str_replace('"'," ",  $data[$cart_item_key]);
				$str = str_replace('\\', '', $nCal);

				$woocommerce->cart->cart_contents[$cart_item_key]['options'][1]['value'] = rtrim($str);
