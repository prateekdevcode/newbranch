...
<div class="restorePop">
	<img class="closePopBtn" src="http://onlinedevserver.biz/dev/breelyn-store/wp-content/themes/breelyn-theme/customproducts/images/cross.png" alt="#">
...
<div class="customize-content">
	<div class="loader" style="display:none;"> <img src="https://breelynuniforms.com.au/wp-content/themes/breelyn/customproducts/images/loader.gif" ></div>
	...
	<div id="id-6" class="content fabric-section text-section">
		...
		<h6>Fabric for Round Neck</h6>
		<select name="fabric_round_nack" id="fabric_round_nack" class="fabric_type_selcet">
			<option value=""> Choose fabric </option>
			<option value="100% Stretchy Polyester">100% Stretchy Polyester</option>
		</select>
		<div class="divider"></div>
		</div>
	</div>
...
jQuery(document).ready(function(){
	...
	jQuery( ".garments-list .eachcat" ).on("click",function(){	
		...
		jQuery.ajax({
			...
			success : function( result ) {
				...
				for (i = 0; i < myJSON.length; i++) {
					html += '<li data-imagename="'+myJSON[i]['data-imagename']+'" data-pid="'+myJSON[i]['products-id']+'" data-price="'+myJSON[i]['data-price']+'" data-layer="'+myJSON[i]['data-layer']+'" data-layers-name="'+myJSON[i]['default']+'" class="design"><a href="#" data-val="'+c+'"><img src="'+myJSON[i]['image']+'" alt="#"><span class="design_name title">'+myJSON[i]['data-imagename']+'</span></a></li>';
					c++;
				}
		...
		if(cat_type=='Traditional Hoodie'){
		...
		if(cat_type=='Tee Shirt Short Sleeve'){
			jQuery( ".fabric_polo_type" ).hide();
			jQuery( ".fabric_hoodie_type" ).hide();
	...
	jQuery(".design-list").on("click","li", function(){	
		...
		var loaderHTML = '<div class="designLoader"><img src="https://breelynuniforms.com.au/wp-content/themes/breelyn/customproducts/images/loader.gif"></div>';
		jQuery('#custom-design-area').find('.customize .designLoader').remove();
		jQuery('#custom-design-area').find('.customize').prepend(loaderHTML);

		jQuery(".variations #design").val(jQuery(this).attr("data-imagename"));
		var data_layer = jQuery(this).attr('data-layer');
		var data_idd = jQuery(this).attr('data-pid');
		...
		console.log(data_layer_name_array);
		var color_pic ='';

		if(jQuery(this).attr("data-imagename") == "New Strike Design"){
			...
			for (var i =1; i <= data_layer; i++)
			{
				...
				if(data_layer_name_array[i-1] == "Side Body"){
					color_pic +='<div class="all-color-data"><span class="color-picker-text"> Pattern 1 </span><input type="text" id="cp'+i+'_getcolor" value="" class="form-control grabdata" placeholder="selected color"><div data-field-name="'+custom_field_name+'" class="colorPicker colorPickSelector'+i+'"></div></div>';
				}else if(data_layer_name_array[i-1] == "Collar"){
					color_pic +='<div class="all-color-data"><span class="color-picker-text">Pattern 2</span><input type="text" id="cp'+i+'_getcolor" value="" class="form-control grabdata" placeholder="selected color"><div data-field-name="'+custom_field_name+'" class="colorPicker colorPickSelector'+i+'"></div></div>';
				}else{
		...
		jQuery.ajax({
			url : '<?php echo admin_url("admin-ajax.php"); ?>',
			data :{action:'get_varietionidof_product', currentProID: data_idd  , fst_var : fst_var, snd_var : snd_var,product_id : '<?php echo get_the_ID();?>'}, 
			type: 'post',
			success : function( result ) {	
					var rsp = JSON.parse(result.pre_design_template);
					console.log(rsp);

				if(rsp){

					if(rsp.product_price ==''){
						rsp.product_price == jQuery('.defaultProductPrice').val();
					}
					...
					jQuery('.restore_savedata').text('Restore done!').attr('disabled',false);
...
jQuery('.save').on('click',function(){
		...
		console.log('data');
		console.log(params_savesettingsdata)
	...
	jQuery.post("<?php echo admin_url('admin-ajax.php'); ?>", {
			...
			function(rsp){				
				...
				jQuery('html, body').animate({
				 ...
				 console.log(JSON.stringify(rsp.raw))
				allData = rsp.raw;
				...
				if(jQuery('.actionA').find('.imgW').length > 0){
					...
					jQuery('.actionA').prepend("<div class='imgW'><img style='width:80%; margin:0 auto 20px auto; ' src="+currenDesign+"></div>");
				}
...
jQuery('.fabric-section select').on('change',function(){
	jQuery('#fabric-details').val(jQuery(this).val());	
});
...
jQuery(window).on('load',function(){
	...
	jQuery('.custom-pocket-list input[type=radio]').on('change',function() {
		if(this.checked) {
