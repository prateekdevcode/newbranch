			...
			foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
				...
				if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_checkout_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
					...
					<?php $product_id = apply_filters( 'woocommerce_cart_item_product_id', $cart_item['product_id'], $cart_item, $cart_item_key ); ?>
						<td data-product-id="<?php echo $product_id; ?>" class="product-thumbnail">
							<?php
	...
	<tfoot>
		...
		<?php if ( wc_tax_enabled() && ! WC()->cart->display_prices_including_tax() ) : ?>
			<?php if ( 'itemized' === get_option( 'woocommerce_tax_total_display' ) ) : ?>
				<?php $i = 0; foreach ( WC()->cart->get_tax_totals() as $code => $tax ) : ?>
					<tr class="tax-rate tax-rate-<?php echo sanitize_title( $code ); ?>">
						<td><?php echo esc_html( $tax->label ); ?></td>
						<?php $i += $tax->formatted_amount; ?>
		...
		<tr class="order-total">
			<td>
				<?php _e( 'Total', 'woocommerce' ); ?> <br>			
			</td>
			<td colspan="2">
				<?php wc_cart_totals_order_total_html(); ?>
				<?php if(WC()->customer->get_shipping_country() == 'AU') { ?>
				<?php
					
					$cartTotal = WC()->cart->total;
					$exvalue = ($cartTotal)/1.1;
				?>
				<span class="cal_gat"> (Including $<?php echo number_format((float)$exvalue*0.1, 2, '.', ''); ?> in taxes) </span>	
				<?php } ?>
			</td>
		</tr>
