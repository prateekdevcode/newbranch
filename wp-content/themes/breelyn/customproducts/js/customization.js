...
var headerHeights = jQuery('#customization-header').outerHeight();
...
jQuery(window).on('load', function() {
    ...
    jQuery('.customization-point').css({ 'height': winHeight - headerHeights }); // window height goes in tools bar
    jQuery('.customize-content').css({ 'height': winHeight - headerHeights }); // window height goes in tools customize bar
    jQuery('.design-area').css({ 'height': winHeight - headerHeights }); // window height goes in main design area
...
jQuery(document).ready(function() {
    ...
    jQuery(document).on('click', '#categories a', function() {
        ...
        categoriesSelected = jQuery('div').data('categoriesSelected');
    ...
    jQuery(document).on('click', '#logo a', function() {
        designSelected = jQuery('div').data('designSelected');
    ...
    jQuery(document).on('click', '#text a', function() {
        ...
        logoSelected = jQuery('div').data('logoSelected');
    ...
    jQuery(document).on('click', '#feature a', function() {
        // jQuery(".demo").customScrollbar();
        ...
        if (logoSelected == undefined) {
            ...
            if (jQuery('#design').hasClass('disabled')) {
                msg = 'Please assign a categories, a design, a logo';
            } else if (jQuery('#logo').hasClass('disabled')) {
                msg = 'Please assign a design, a logo';
            } else {
                msg = 'Please assign a logo first';
            }
            openPopup(msg);
            return false;
        } else {
    ...
    jQuery(document).on('click', '#fabric a', function() {
    ...
    jQuery(document).on('click', '#size-quantity a', function() {
    ...
    jQuery(document).on('click', '#finalization a', function() {
    ...
    /* ***** add to cart section Section ***** */


    jQuery(document).on('click', '#cart a', function() {

        logoSelected = jQuery('div').data('logoSelected');
        txtDataVal = jQuery('div').data('sizeSelected');
        if (logoSelected == undefined) {

            if (jQuery('#design').hasClass('disabled')) {
                msg = 'Please assign a categories, a design, a logo and size and quantity';
            } else if (jQuery('#logo').hasClass('disabled')) {
                msg = 'Please assign a design, a logo and size and quantity';
            } else {
                msg = 'Please assign a logo and size and quantity first';
            }
            openPopup(msg);
            return false;
        } else if (txtDataVal == undefined) {
            msg = 'Please assign Size and Quantity';
            openPopup(msg);
            return false;
        } else {
    ...
    jQuery('#text-string').on('change', function() {
        //alert('change');
        jQuery(this).siblings('button.assign').removeAttr('disabled', '');
    });

    jQuery('#text-string-back').on('change', function() {
        //alert('change');
        jQuery(this).siblings('button.assign').removeAttr('disabled', '');
    });
    ...
    jQuery('.assign').on('click', function() {

        var inputVal = jQuery(this).siblings('input[name="name"]').val();
        //alert(inputVal);
        if (!inputVal == "") {
            //alert('full');
            jQuery(this).siblings('input[type="hidden"]').val(inputVal)
        } else {
            //alert('blank');
            return false;
        }
    ...
    jQuery(document).ajaxStop(function() {
        ...
        if (jQuery(document).find('.data_restored .custom-logo-list li').length > 0) {
            var proId = jQuery(document).find('.custom-logo-list li').eq(0).find('a').attr('data-val');

            jQuery('div').data('logoSelected', proId);
            jQuery('#text').removeClass('disabled');
            jQuery('#feature').removeClass('disabled');
            jQuery('#fabric').removeClass('disabled');
            jQuery('#size-quantity').removeClass('disabled');
            jQuery('#finalization').removeClass('disabled');
    ...
    jQuery(document).on('click', '.customize-content .content ul li', function(e) {

        e.preventDefault();
        proId = jQuery(this).children().attr('data-val');
    ...
    jQuery(document).on('change', '.customize-content .content ul.size li input[type=number]', function(e) {


        var txtDataVal = jQuery(this).val();
        //alert(txtDataVal); 
        if (txtDataVal > 0) {
            jQuery('div').data('sizeSelected', txtDataVal);

            jQuery('#cart').removeClass('disabled');
        }
    });
    ...
    jQuery('.garments-list').on('click', 'li', function() {
        jQuery('.design-list').fadeIn(500);
        jQuery('.color-picker-container').fadeOut(500);
        jQuery('.back').css({ 'opacity': '0', 'transition': 'all .5s ease-in-out' });
    })
    ...
    jQuery(document).on('click', '.cross-img', function(e) {
        ...
        var file = jQuery(this).parent().parent().next('div').find('input[type=file]').val('');
