<script>
	...
	//     var r = confirm("Leave the page without saving the design?");
	// 		if (r == true) {
	// 		  //delete
	// 		  return 'true';
	// 		} else {
...
preventNavigation("Leave the page without saving the design?");
</script>
<?php
...
.actionA ul li span:first-child{
	height: 76px;
    overflow: hidden;
    position: relative;
    width: 130px;
}
.actionA ul li span:first-child img{
	max-width: 130px;
    position: absolute;
    top: 50%;
...
<section id="custom-design-area" class="clearfix">
	<div class="design-area  parent">
        ...
        <div class="right-side-bar">
        		...
        		<div class="prices">
					<table>
						...
						<tr data-symbol="<?php echo get_woocommerce_currency_symbol();?>" data-price="42.13" data-qty="10-24">
							<td>-</td>
							<td><?php echo get_woocommerce_currency_symbol();?>-</td>
						</tr>
			...
			<input type="hidden" id="client_ip" value="<?php echo get_client_ip();  ?>">
			<p class="setting_status_message" style="font-size: 15px;font-weight: bold;padding: 6px;margin-top: 5px;border-radius: 10px;"></p>
				...
				<button class="downloadDesign">Download Design</button>
...
<div class="customize-content">
	...
	<div id="id-5" class="content text-section">
		...
		<div class="feature_3">
			<h6>Knitted Cuff</h6>
			<p>(MCQ 100 for Long Sleeve Polo)</p>
			<div class="custom-pocket-list">
				<div><label><input data-price="2.2" type="radio" value="Knitted-cuff" name="Knitted_cuff"> Knitted Cuff <span> ( +$2.2 each ) </span> </label></div>
				<div><label><input data-price="0" type="radio" value="no-pocket" name="Knitted_cuff" checked> No Knitted Cuff  </label></div>
			</div>

			<h6>Knitted Collar</h6>
			<p>(MCQ 100 for Polo Shirts)</p>
			<div class="custom-pocket-list">
				<div><label><input data-price="2.2" type="radio" value="Knitted-collar" name="Knitted_collar"> Knitted collar <span> ( +$2.2 each ) </span> </label></div>
				<div><label><input data-price="0" type="radio" value="no-pocket" name="Knitted_collar" checked> No Knitted collar  </label></div>
			</div>
		</div>
	</div>
...
jQuery('.save').on('click',function(){
		...
		var client_ip = jQuery('#client_ip').val();
		...
		console.log(JSON.stringify(params_savesettingsdata));
			...
			//jQuery('body').append("<img src='"+ base64image +"'>");
...
jQuery('.downloadDesign').on('click',function(){
...
function setFeatures(selected_category){
	if(selected_category.slug == 'polo'){

		jQuery('.feature_1').show();
		jQuery('.feature_2').hide();
		jQuery('.feature_3').hide();

	}else if(selected_category.slug == 'round-neck'){

		jQuery('.feature_1').show();
		jQuery('.feature_2').hide();
		jQuery('.feature_3').hide();

	}else if(selected_category.slug == 'pull-over'){

		jQuery('.feature_1').hide();
		jQuery('.feature_2').hide();
		jQuery('.feature_3').show();
...
function makeimg(){
	...
	var base64image = canvas.toDataURL("image/png");
	 ...
	 var pro_id = jQuery(document).find('.variations_form.cart').attr('data-product_id');

	  localStorage.setItem('pro_img_custom_'+pro_id,base64image)    	
	  //jQuery('body').append("<img style='width:110px; margin:0 auto 20px auto; ' src="+localStorage.getItem('pro_img_custom')+">");
...
function getDesignItems(){
	...
	jQuery.ajax({
			url : '<?php echo admin_url("admin-ajax.php"); ?>',
			data :{action:'getselectproductstyle', cat_slug : selected_category.slug, post_id : <?php echo get_the_ID(); ?>}, 
			type: 'post',
			success : function( result ) {
				...
				for (i = 0; i < design_data.length; i++) {
					html += '<li data-variationid="'+design_data[i]['product_variation']+'" data-imagename="'+design_data[i]['data-imagename']+'" data-pid="'+design_data[i]['products-id']+'" data-price="'+design_data[i]['data-price']+'" data-layer="'+design_data[i]['data-layer']+'" data-layers-name="'+design_data[i]['default']+'" class="design"><a href="#" data-val="'+c+'"><img src="'+design_data[i]['image']+'" alt="#"><span class="design_name title">'+design_data[i]['data-imagename']+'</span></a></li>';
					c++;
				}
...
var product_variation = currentItem.attr('data-variationid');

jQuery('.variation_id').val(product_variation);
	...
	var fileName = selected_design.name.toLowerCase().replace(/ /g,'_');
	 ...
	 var price_table = jQuery('.prices');
	 price_table.find('tbody').html('')
	 var html_rules = '';
	 for (var i = 0; i < price_rules.length; i++) {
	 	if(price_rules[i].id == fileName.toUpperCase()){
	 		
	 		console.log(price_rules[i].rules.length)

	 		for(var j = 0; j< price_rules[i].rules.length; j++){
	 			console.log(price_rules[i].rules[j])
	 			html_rules = html_rules + '<tr data-symbol="'+ price_rules[i].rules[j].symbol +'" data-price="'+ price_rules[i].rules[j].price +'" data-qty="'+ price_rules[i].rules[j].range +'"><td>'+ price_rules[i].rules[j].range +'</td><td>'+ price_rules[i].rules[j].symbol + price_rules[i].rules[j].price +'</td></tr>';
	 		}
	 		console.log(html_rules)
	 	}
	 }

	 price_table.find('tbody').append(html_rules)
