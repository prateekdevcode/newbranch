...
add_action( 'woocommerce_after_single_product_summary', 'tst_your_function' );
    ...
    global $woocommerce;
		...
		$the_query = new WP_Query( array(
					 ...
					 if ($the_query->have_posts()) : 
						...
						$geturlofproduct = get_the_post_thumbnail_url( $post = $product_id, $size = 'full' );?>
						
						<img src="<?php echo $geturlofproduct; ?>" alt=""/>
						<?php 
						the_title();
...
//print_r($product); 
//$x = $product->get_downloadable( $product_id );
//print_r($x);
/* global $woocommerce,$product;
$id = $product->get_id();
print_r($id); */
...
function custom_permalink( $permalink, $post, $leavename ) {
	...
	print_r($category);
   // if (  !empty($category) && $category[0]->cat_name == "greenway" ) {
      //  $cat_name = strtolower($category[0]->slug);
        $permalink = trailingslashit( home_url('/post/'. $cat_name . '/' . $post->post_name .'/' ) );
    //}
    return $permalink;
}
...
function custom_permalink( $permalink, $post, $leavename ) {
 ...
 // if (  !empty($category) && $category[0]->cat_name == "greenway" )
 // {
      $permalink = trailingslashit( home_url('post/'. $category[0]->slug . '/' . $post->post_name ) );
 /// }
 }
   return $permalink;
...
{
  $newrules = array();
  $newrules['^post/' . $category[0]->slug. '([^/]*)/([^/]*)/?'] = 'index.php?$category=$matches[1]&name=$matches[2]';
...
function post_permalink_w_blog( $link ) {
  ...
  if ( $post->post_type === 'post' ) {
    $link = str_replace( $post->post_name, 'post/' . $post->post_name, get_permalink( $post ) );
  }
  return $link; 
}